...
func determine_breakable_positions() -> void:
	...
	# 	if tilemap.tile_set.tile_get_name(tilemap.get_cell_source_id(0,cell)) == "full tilemap.png 10":
	# 		breakable_positions.append(tilemap.map_to_local(cell))
...
func spawn_breakables(count: int) -> void:
	...
	for _i in range(count):
		...
		add_child(breakable)
		breakable_positions.remove_at(position_index)  # Eliminar la posición para evitar spawns superpuestos	
...
func _close_entrance() -> void:
	for entry_position in entrance.get_children():
		tilemap.set_cell(0,tilemap.local_to_map(entry_position.position), 1)
		tilemap.set_cell(0,tilemap.local_to_map(entry_position.position) + Vector2i.DOWN, 2)
